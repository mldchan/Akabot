...
class Welcoming(discord.Cog):
    ...
    async def on_member_remove(self, member: discord.Member):
        target_channel_id = get_setting(self.db, member.guild.id, 'goodbye_channel', '0')
        if target_channel_id == '0':
            return # The channel was not set and was initialised as we just ran this
        message_type = get_setting(self.db, member.guild.id, 'goodbye_type', 'embed') # embed or text
        message_title = get_setting(self.db, member.guild.id, 'goodbye_title', 'Goodbye') # embed title (if type is embed)
        message_text = get_setting(self.db, member.guild.id, 'goodbye_text', 'Goodbye {user}!') # text of the message or description of embed
