...
class ChatSummary(discord.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        db = await get_conn()
        cur = await db.execute('SELECT * FROM chat_summary WHERE guild_id = ? AND channel_id = ?',
        ...
        await db.execute('UPDATE chat_summary SET messages = messages + 1 WHERE guild_id = ? AND channel_id = ?',
                    (message.guild.id, message.channel.id))

        # Increment message count for specific member
        cur = await db.execute('SELECT * FROM chat_summary_members WHERE guild_id = ? AND channel_id = ? AND member_id = ?',
    ...
    async def command_add(self, ctx: discord.ApplicationContext, channel: discord.TextChannel):
        ...
        # Check enabled state
        cur = await db.execute('SELECT enabled FROM chat_summary WHERE guild_id = ? AND channel_id = ?',
                    (ctx.guild.id, channel.id))
        ...
        # Enable
        await db.execute('UPDATE chat_summary SET enabled = 1 WHERE guild_id = ? AND channel_id = ?',
                    (ctx.guild.id, channel.id))
        await db.commit()
        await db.close()
    ...
    async def command_remove(self, ctx: discord.ApplicationContext, channel: discord.TextChannel):
        ...
        # Set disabled
        await db.execute('UPDATE chat_summary SET enabled = 0 WHERE guild_id = ? AND channel_id = ?',
                    (ctx.guild.id, channel.id))
        await db.commit()
        await db.close()
    ...
    async def summary_dateformat(self, ctx: discord.ApplicationContext, date_format: str):
        ...
        logging_embed.add_field(name=await trl(0, ctx.guild.id, "chat_summary_dateformat_log_dateformat"),
    ...
    async def chatsummary_countedits(self, ctx: discord.ApplicationContext, countedits: bool):
        ...
        # Respond
        await ctx.respond(
            await trl(ctx.user.id, ctx.guild.id, "chat_summary_count_edits_on", append_tip=True) if countedits else
            await trl(ctx.user.id, ctx.guild.id, "chat_summary_count_edits_off", append_tip=True),
            ephemeral=True)
