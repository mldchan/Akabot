...
class Suggestions(discord.Cog):
    ...
    async def cmd_add_channel(self, ctx: discord.ApplicationContext, channel: discord.TextChannel):
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, 'suggestions_channel_added').format(channel=channel.mention),
                          ephemeral=True)
    ...
    async def cmd_remove_channel(self, ctx: discord.ApplicationContext, channel: discord.TextChannel):
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, "suggestions_channel_removed").format(channel=channel.mention),
                          ephemeral=True)
    ...
    async def cmd_message_reminder(self, ctx: discord.ApplicationContext, enabled: bool, message: str):
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, 'suggestions_message_reminder_set').format(enabled=enabled, message=message), ephemeral=True)