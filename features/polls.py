    ...
    def save(self):
        def insert_votes():
            for member_id in self.ans1_votes:
                cur.execute('select * from polls_answers where guild_id = ? and message_id = ? and member_id = ?',
        ...
        if cur.fetchone():
            ...
            cur.execute('insert into polls (guild_id, message_id, ans1, ans2, ans3, ans4, ans5, question, closed) '
                        'values (?, ?, ?, ?, ?, ?, ?, ?, ?)', (
