...
class ViolationCounters():
    ...
    def add_action(self, action: str, user: discord.Member, expires: int):
        ...
        self.past_actions.append(
            {'action': action, 'user': user, 'expires': expires + time.time()})

    def filter_expired_actions(self):
        self.past_actions = [
            action for action in self.past_actions if action['expires'] > time.time()]
...
class AntiRaid(discord.Cog):
    ...
    antiraid_subcommand = discord.SlashCommandGroup(
        name='antiraid', description='Manage the antiraid settings')
    ...
    async def list_settings(self, ctx: discord.Interaction):
        join_threshold = get_setting(
            ctx.guild.id, 'antiraid_join_threshold', 5)
        join_threshold_per = get_setting(
            ctx.guild.id, 'antiraid_join_threshold_per', 60)

        embed = discord.Embed(title='Antiraid settings',
                              color=discord.Color.blurple())
        embed.add_field(name='Join threshold',
                        value=f'{join_threshold} per {join_threshold_per} seconds')
