...
class AntiRaid(discord.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if self.violation_counters.count_actions('join', member) > 5:
            if member.can_send():
                await member.send(content="You have been kicked from the server for suspected raiding. If you believe this was a mistake, try rejoining in a few minutes.")
            await member.kick(reason='Raiding')
            return
