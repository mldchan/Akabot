...
class BirthdayAnnouncements(discord.Cog):
    ...
    async def set_channel(self, ctx: discord.ApplicationContext, channel: discord.TextChannel):
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, "birthday_announcements_channel_set", append_tip=True).format(channel=channel.mention), ephemeral=True)
    ...
    async def handle_birthday_announcements(self):
        ...
        for guild in self.bot.guilds:
            birthday_announcements_channel = int(get_setting(guild.id, "birthday_announcements_channel", "0"))
            if birthday_announcements_channel == 0:
                continue

            now = get_now_for_server(guild.id)
            if now.hour != 12 or now.minute != 0:
                continue

            birthdays_this_day = search_settings_by_value(date_string)
            for i in birthdays_this_day:
                ...
                if get_per_user_setting(member.id, f'birthday_announcements_{str(guild.id)}', 'true') == 'true':
                    message = get_setting(guild.id, "birthday_announcements_message", "Today is {mention}'s birthday! 🎉 Say happy birthday! 🎂")
                ...
                if get_per_user_setting(member.id, 'birthday_send_dm', 'false') == 'true':
                    try:
                        if get_per_user_setting(member.id, 'birthday_reveal_age', 'false'):
                            await member.send(trl(member.id, 0, "birthday_dm_age").format(age=str(now.year - int(i[1][:4]))))
                        else:
