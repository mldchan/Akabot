...
class RolesOnJoin(discord.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        roles = client['RolesOnJoin'].find({'GuildID': member.guild.id})
        for row in roles:
            ...
            if role is not None:
                ...
                # remove
                client['RolesOnJoin'].delete_one({'GuildID': member.guild.id})
    ...
    async def add_role_on_join(self, ctx: discord.ApplicationContext, role: discord.Role):
        ...
        if client['RolesOnJoin'].count_documents({'GuildID': ctx.guild.id, 'RoleID': role.id}) == 0:
            client['RolesOnJoin'].insert_one({'GuildID': ctx.guild.id, 'RoleID': role.id})
    ...
    async def remove_role_on_join(self, ctx: discord.ApplicationContext, role: discord.Role):
        if client['RolesOnJoin'].count_documents({'GuildID': ctx.guild.id}) == 0:
            await ctx.respond(trl(ctx.user.id, ctx.guild.id, "roles_on_join_role_not_in_join_roles"), ephemeral=True)
        else:
            client['RolesOnJoin'].delete_one({'GuildID': ctx.guild.id})
    ...
    async def list_roles_on_join(self, ctx: discord.ApplicationContext):
        rows = client['RolesOnJoin'].find({'GuildID': ctx.guild.id})
        if not rows:
        ...
        for row in rows:
            ...
            if role is None:
                client['RolesOnJoin'].delete_one({'GuildID': ctx.guild.id, 'RoleID': row['RoleID']})
                continue
            msg = f"{msg}\n{role.mention}"
