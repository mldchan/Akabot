...
class PerUserSettings(discord.Cog):
    ...
    async def set_birthdate(self, ctx: discord.ApplicationContext, birthday: str):
        ...
        if int(birthday[:4]) < 1900 or int(birthday[:4]) > get_now_for_server(ctx.guild.id).year - 14 or int(
                birthday[5:7]) < 1 or int(birthday[5:7]) > 12 or int(birthday[8:]) < 1 or int(
                birthday[8:]) > 31:
            await ctx.respond(trl(ctx.user.id, ctx.guild.id, "per_user_birthday_invalid"), ephemeral=True)
            return
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, "per_user_birthday_set").format(birthday=birthday),
                          ephemeral=True)
    ...
    async def set_birthday_settings(self, ctx: discord.ApplicationContext, reveal_age: bool | None = None,
        ...
        reveal_age = trl(ctx.user.id, ctx.guild.id, "per_user_birthday_reveal_age_enabled") if reveal_age else trl(
            ctx.user.id, ctx.guild.id, "per_user_birthday_reveal_age_disabled")

        send_dm = trl(ctx.user.id, ctx.guild.id, "per_user_birthday_send_dm_enabled") if send_dm else trl(ctx.user.id,
                                                                                                          ctx.guild.id,
    ...
    async def set_birthday_announcement(self, ctx: discord.ApplicationContext, state: bool):
        ...
        if state:
            state_str = trl(ctx.user.id, ctx.guild.id, "per_user_birthday_announcements_enabled")
        else:
            state_str = trl(ctx.user.id, ctx.guild.id, "per_user_birthday_announcements_disabled")
    ...
    async def clear_birthday(self, ctx: discord.ApplicationContext):
        ...
        await ctx.respond(trl(ctx.user.id, ctx.guild.id, "per_user_birthday_cleared"), ephemeral=True)
